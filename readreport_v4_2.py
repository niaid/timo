...
def printer(outfile, sampname, SEGMENT, ntpos, bcheck, hasMinor): #this is how the output data looks 'pretty'
    ...
    tempd = analyzer.CONSENSUS_DICT[ntpos]
    A_nt = 0
    C_nt = 0
    G_nt = 0
    T_nt = 0
...
if __name__ == '__main__': #this will allow this module to be imported from another module and used
    ...
    for sampctr, SAMPLEFILE in enumerate(SAMPLELIST):
        ...
        for segidx,SEGMENT in enumerate(REF_DICT): #enumerate segidx if you print will give you the index
            ...
            #these are cap making fresh for dict building dictionaries see more above
            analyzer.setupDictionaries(SEGLEN)
            counter = 0
            try:
                for read in BAMFILE.fetch(SEGMENT):
                    if read.is_unmapped:
                        ...
                        analyzer.analyzer(isReverse,
                                analyzer.seqUpdater(cigartup,unfiltread,
                                    unfiltreadidx,unfiltreadqual),
                                args.qual
                                )
                        # createCombineDict()
                sys.stdout.write('[done]\n')
                sys.stdout.write('\tAnalyzing frequencies and binomial checks ...')
                for ntpos in sorted(analyzer.CONSENSUS_DICT.keys()): #looking at each nt position going down
                    totalcount = sum(analyzer.CONSENSUS_DICT[ntpos].values())
                    posdict = {k: v / float(totalcount) for k, v in analyzer.CONSENSUS_DICT[ntpos].items()}
                    if posdict: #If posdict empty, this means there is no coverage
                        ...
                        if maxfreq >= (1 - args.cutoff): # indicates no "significant" minor variant
                            ...
                            binomPass = analyzer.binomCheck(ntpos, args.cutoff)
